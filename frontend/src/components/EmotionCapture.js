...
function EmotionCapture() {
  ...
  const [musicPref, setMusicPref] = useState("Global");
  ...
  // Button click function explicitly defined
        ...
        <div style={{ marginTop: '20px', textAlign: 'left' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '350px', overflowY: 'auto', paddingRight: '5px' }}>
            {songs.map((song, index) => (
              <div key={index} className="song-card" style={{ display: 'flex', alignItems: 'center', padding: '10px', borderRadius: '12px', background: theme.palette.mode === 'dark' ? '#B0E0E6' : '#f9f9f9', border: `1px solid ${theme.palette.mode === 'dark' ? '#D6F5FF' : 'rgba(0,0,0,0.05)'}`, transition: 'all 0.2s ease' }}
                ...
                <button 
                  ...
                  style={{ padding: '8px 16px', background: `linear-gradient(135deg, ${theme.palette.primary.main}, ${theme.palette.secondary.main})`, color: 'white', border: 'none', borderRadius: '999px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px', boxShadow: '0 4px 10px rgba(0,0,0,0.2)' }}
                >
                  Play Full
                </button>
              </div>
